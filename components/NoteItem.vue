<template>
  ...
  >
    <a-col :span="props.note.cover?17:23">
      ...
      <a-row class="meta">
        <a-col class="jsd-meta"><icon name="ion:diamond"/>136.0</a-col>
        <NuxtLink class="nickname">{{props.note.nickname}}</NuxtLink>
        <NuxtLink class="comments"><icon name="mdi:message"/> 10</NuxtLink>
        <NuxtLink class="like" @click="likeClick">
          <icon name="mdi:cards-heart" :style="{color:props.note.flag?'red':''}" />{{props.note.like}}
        </NuxtLink>
      </a-row>
    </a-col>
    <a-col :span="1"></a-col>
